...
function trigger_promocode(player, code)
        ...
        if code == "BesteLebenMate" then 
            if getElementData(player, "promocode") == 0 then
                setPremiumData (player, 2,2)
                vnxSetElementData ( player, "socialState", "Beste Leben" )
                setElementData(player, "promocode", "1")
                dbExec ( handler, "UPDATE ?? SET ?? = ? WHERE ??=?", "userdata", "promocode", "1", "UID", playerUID[name] )
                outputDebugString(PremiumMomentan)
            else outputChatBox("Du hast schon einen Promotion Code eingelöst." , player, 0, 105, 145 ) end 
        elseif code == "FortniteMoves" then 
            if getElementData(player, "promocode2") == 0 then
        ...
        elseif code == "SanAndreas" then 
            if getElementData(player, "promocode3") == 0 then
        ...
        elseif code == "PremiumIstNice" then 
            if getElementData(player, "promocode4") == 0 then
                setElementData(player, "promocode4", "1") 
                setPremiumData (player, 3,4)
                ...
                outputChatBox("[Promotion Code] : Du hast zusätzlich noch VIP Platin für 3 Tage bekommen :).", player, 0,255,0)
                vnxSetElementData ( player, "money", vnxGetElementData ( player, "money" ) + 14000 ) 
            else outputChatBox("Du hast schon einen Promotion Code eingelöst." , player, 0, 105, 145 ) end 
        elseif code == "Layesd_tv" then  
            if getElementData(player, "promocode5") == 0 then
        ...
        elseif code == "pedophil" then
            if getElementData(player, "promocode6") == 0 then 
                setElementData(player, "promocode6", "1") 
                giveAchievement( player, 20 )
                ...
                outputChatBox("Du hast nun einen neuen Sozialen Status : NeverSupportPedos", player , 0,255,0)
                vnxSetElementData ( player, "socialState", "NeverSupportPedos" )      
            else outputChatBox("Du hast schon einen Promotion Code eingelöst." , player, 0, 105, 145 ) end 
        else 
            outputChatBox("Der Eingegebene Code ist ungültig." , player, 125, 0, 0 )
            
        end
        if code == "VnX2017" or code == "DennisPlayTennis" or code == "TimPetaaa"  or code == "LKWFahrer" or code == "Solid_Snake" then 
            outputChatBox("Der Eingegebene Code ist abgelaufen." , player, 125, 0, 0 )
        --else 
           -- outputChatBox("Der Eingegebene Code ist ungültig." , player, 125, 0, 0 )
        end
end 
...
local randomlorenzotexte = {
    function(player)
        outputChatBox("[Lorenzo Flüstert] : Pschhhhht.... Ich bin ein Lorenzo... Hast du kleine Kinder für mich ?", player, 150, 0, 0 )
    end,
    function(player)
        outputChatBox("[Lorenzo Flüstert] : Ich will Kleine Kinder... BITTE ... ", player, 150, 0, 0 )
    end,
    function(player)
        outputChatBox("[Lorenzo Flüstert] : Uff ist Venox Geil... Erstmal nachmachen auf lese rechtschreibschwäche Reallife :O", player, 0, 150, 255 )
    end,
    function(player)
        outputChatBox("[Lorenzo Flüstert] : Venox ist keine Konkurrenz :D", player, 200, 200, 200 )
    end,
    function(player)
        outputChatBox("[Lorenzo Flüstert] : Ich werde von meinem eigenen Projekt gederanked... :/ ", player, 200, 200, 200 )
    end,
    function(player)
        outputChatBox("[Lorenzo Flüstert] : Ich werde Alle F*cken mit meinem Selfmade :D", player, 0, 200, 255 )
    end,
    function(player)
        outputChatBox("[Random Venox Spieler] : Selfmade...? Ich dachte du hast alles selber gemacht :O ... ", player, 220, 220, 220 )
        outputChatBox("[Lorenzo] : Selfmade... Alles SELBER gedownloadet und bei Solid_Snake gelutscht :)", player, 200, 200, 200 )
    end,
    function(player)
        outputChatBox("[Lorenzo Flüstert] : Ich bin kein Rassist... Bonus du Fetter Türke... Solid_Snake du scheiß Perser ... Ihr SCHEIß AUSLÄNDER!!!111!!!!111eins!!elf!", player, 125, 0, 0 )
    end,
    function(player)
        outputChatBox("[Lorenzo] : Was ?! Venox hat mehr Spieler als Leserechtschreibschwäche Reallife?? Erstmal Ddosen :D", player, 125, 0, 0 )
    end,
    function(player)
        outputChatBox("[Lorenzo] : Fragen wir mal meine User ob sie für einen Team-Rang mir Kinderpornos schicken :O ", player, 125, 0, 0 )
    end,
}

local function lorenzopedoinfo(player)
    randomlorenzotexte[math.random(1, #randomlorenzotexte)](player)
end 

local lorenzoderpedo = createPed(277,-1628.0498046875 , -2210.919921875 , 29.0,90,true)
setElementFrozen(lorenzoderpedo,true)
setTimer(setElementHealth,51,0,lorenzoderpedo,100)



local lorenzopedomarker = createCustomPickup(-1629.4287109375 , -2210.7783203125 , 30.049606323242, 0, 0, "anzeigen/achievments/achievmentPictures/pedo.png", lorenzopedoinfo)