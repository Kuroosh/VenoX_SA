...
local duty_marker = createMarker(-2391.3823, -189.9117, 34.28888, "cylinder", 1.6, 0, 120, 200, 255)
	...
	if(isMechaniker(thePlayer) or isMedic(thePlayer)) and (isElementWithinMarker(thePlayer, duty_marker)) then
		...
		if(duty == true) then
			...
			if isMedic(thePlayer) then
				vnxSetElementData (thePlayer, "fraktion", 11)
			end
			...
			vnxSetElementData(thePlayer, "repairkits", 5)
		end
	end
end)
...
function delete_mech_blip (player, thePlayer)
	...
	vnxSetElementData (player, "needMech", false)
end
...
addCommandHandler("repaircar", function(thePlayer, cmd)
		if(isMechaniker(thePlayer)) and (isEmergencyOnDuty(thePlayer)) then
			...
			local kits = tonumber(vnxGetElementData(thePlayer, "repairkits"))
			...
			if vehicle then
				...
				if kmh < 20 then
					...
					setTimer(function(thePlayer, vehicle)
						...
						vnxSetElementData(thePlayer, "repairkits", kits-1)
						fixVehicle(vehicle)
						if getVehicleOccupant(vehicle) ~= thePlayer then
