...
float getCosine(float coord)
{
	return saturate(abs(cos( coord * pi )));
}
...
PSInput VertexShaderFunctionDB(VSInput VS)
{
	...
	float4 worldPos = mul(position, gWorldFix );
	float3 posOffset = normalize(worldPos.xyz - gCameraPosition );
    ...
    worldViewPosition.xyz *= saturate( fViewMult );	
...
float4 PixelShaderFunctionDB(PSInput PS) : COLOR0
{
    ...
    float fade = saturate( ( depth - PS.DistFromCam * fDistMult + fdistOffset ) * fDepthSpread);
	
    float4 color = tex2D( Sampler0 , PS.TexCoord.xy );
    color.rgb = pow( color.rgb * 1.2, 1.5);
    color *= saturate( float4( PS.Diffuse.rgb, 1 ));
