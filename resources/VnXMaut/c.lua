	...
	addEventHandler("onColShapeHit", col, function(hitElement)
		if(getElementType(hitElement) == "vehicle") and (getVehicleOccupant(hitElement)) then
			...
			if not(getVehicleSirensOn(hitElement)) then
				...
				if(getElementData(source, "weiterfahrt") ~= true) then
				---
					laSetElementData (thePlayer, "money", money - mautstellenpreis)
			...
			setTimer(function()
						...
						local wanteds = laGetElementData (thePlayer, "wanteds")
						local strecke
						if (id == 1) then
					...
					if laGetElementData ( hitElement, "illegalcar") == 1 then
						...
						if(rand == 5) then
							...
							if wanteds < 6 then
								laSetElementData (thePlayer, "wanteds", wanteds + 1)
