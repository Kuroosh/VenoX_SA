...
function aChatTab.Create(tab)
    ...
    if (aGetSetting("adminChatSound")) then
        guiCheckBoxSetSelected(aChatTab.AdminChatSound, true)
    end
...
function aChatTab.onClientAdminChat(message)
    ...
    if (guiCheckBoxGetSelected(aChatTab.AdminChatOutput)) then
        outputChatBox("ADMIN> " .. getPlayerName(source) .. ": " .. message, 255, 0, 0)
    end
    if ((guiCheckBoxGetSelected(aChatTab.AdminChatSound)) and (source ~= getLocalPlayer())) then
