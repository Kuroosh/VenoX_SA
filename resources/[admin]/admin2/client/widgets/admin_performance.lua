...
*
*	client\widgets\admin_performance.lua
*
...
function aPerformance()
    if (aPerformanceForm == nil) then
        ...
        aPerformanceLabel =
            ...
            0.74,
            0.18,
            "It's recomended to unload the widgets you do not often use to save RAM",
            true,
            aPerformanceForm
        )
        guiLabelSetHorizontalAlign(aPerformanceLabel, "left", true)
        guiLabelSetColor(aPerformanceLabel, 255, 0, 0)
        aPerformanceSpectator =
            guiCreateCheckBox(0.05, 0.10, 0.90, 0.08, "Unload spectator when not used", false, true, aPerformanceForm)
        aPerformanceEditor =
            guiCreateCheckBox(0.05, 0.18, 0.90, 0.08, "Unload map editor when not used", false, true, aPerformanceForm)
        aPerformanceTeam =
            guiCreateCheckBox(0.05, 0.26, 0.90, 0.08, "Unload Team Form when not used", false, true, aPerformanceForm)
        aPerformanceSkin =
            guiCreateCheckBox(0.05, 0.34, 0.90, 0.08, "Unload Skin Form when not used", false, true, aPerformanceForm)
        aPerformanceStats =
            guiCreateCheckBox(0.05, 0.42, 0.90, 0.08, "Unload Stats Form when not used", false, true, aPerformanceForm)
        aPerformanceVehicle =
            guiCreateCheckBox(
            0.05,
            0.50,
            0.90,
            0.08,
            "Unload Vehicle Form when not used",
            false,
            true,
            aPerformanceForm
        )
        ...
        aPerformanceMessage =
            guiCreateCheckBox(0.05, 0.78, 0.90, 0.08, "Unload Message Box when not used", false, true, aPerformanceForm)
