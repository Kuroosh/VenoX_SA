    ...
    if (button == "left") then
        ...
        elseif (source == aACLCreateGroup) then
            aInputBox(
                ...
                "Enter group name:",
                "",
                'triggerServerEvent ( "aAdmin", getLocalPlayer(), "aclcreate", "group", $value )'
            )
        ...
        elseif ((source == aACLRemoveObject) or (source == aACLAddACL) or (source == aACLRemoveACL)) then
            ...
            if (source == aACLRemoveObject) then
                table = aAclData["group_objects"][aAclData["current"]]
            elseif (source == aACLAddACL) then
                table = aAclData["acl"]
            elseif (source == aACLRemoveACL) then
                table = aAclData["group_acls"][aAclData["current"]]
            end
            if (#table >= 1) then
                guiSetText(aACLDropCurrent, table[1])
            end
