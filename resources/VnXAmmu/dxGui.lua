...
local dxfont0_drugs = dxCreateFont(":venox/fonts/drugs.ttf", 15)
local dxfont1_roboto = dxCreateFont(":venox/fonts/Roboto-Thin.ttf", 10)
...
function vnxDrawElements()
	...
	for _,v in ipairs (edits) do 
		...
		if not v.sel then
			dxDrawRectangle(x,y,sx,20,tocolor(10,10,10,255)) --editBox?
		else
			dxDrawRectangle(x,y,sx,20,tocolor(56,56,56,200))
		...
		if v.sel then
			dxDrawText(txt,x+2,y,x+sx-4,y+20,tocolor(255,255,255,255),1, "default","left","center",true)
		else 
			dxDrawText(txt,x+2,y,x+sx-4,y+20,tocolor(255,255,255,255),1, "default","left","center",true)
		end
		if v.carshow < 0.5 and v.sel then
			local x = x+2+dxGetTextWidth(string.sub(txt,1,v.car),1,"default")
			dxDrawLine(x,y+2,x,y+20-3,tocolor(255,255,255,255),1)
		end
	end
	for _,v in ipairs (texts) do
		...
		dxDrawText(txt,x,y,x+sx,y+sy,v.c,v.s, "default",v.ax,v.ay,true)
		if #v.t > v.maxr then
