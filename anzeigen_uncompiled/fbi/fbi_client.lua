...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function dutyWindow_fbi()
	if getElementData(getLocalPlayer(),"loggedin") == 1 then  
		if getElementData(getLocalPlayer(),"fraktion") == 6 then
			...
			showCursor(true)
			vnxDrawWindow("duty_window","window","Willkommen Agent "..name,x,y,500,400,"close")
			vnxDrawButton("SWAT","button","S.W.A.T",z,u,170,80,"swat_func_fbi")
			vnxDrawButton("duty","button","DUTY",c,v,170,80,"duty_func_fbi")
			vnxDrawButton("offdutybutton","button","OFF-DUTY",e,f,170,80,"offduty_func_fbi")
...
addEvent("fbi_window_client",true)
addEventHandler("fbi_window_client",getRootElement(),dutyWindow_fbi)
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function duty_func_fbi( player )
	local player = getLocalPlayer()
		...
		local name = getPlayerName( player )
		showCursor(true)
		triggerServerEvent( "fbi_duty_window_btn", getLocalPlayer(), getLocalPlayer() )
		setElementFrozen( getLocalPlayer(), true )
	if tonumber(getElementData( player, "rang" )) >= 0 then	
		vnxDrawWindow("duty_window_weapons","window","Wählen sie ihre Waffen für ihren Einsatz aus",x-40,y-40,555,520,"close")
